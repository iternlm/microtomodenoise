...
#include "Denoiser/iternlm_gpu.h"
...
int main(int argc, char* argv[])
{
	...
	if ("extract command line arguments)"){
		for (uint16_t i = 1; i < argc; i++)
		{
			...
			else if (string(argv[i]) == "--preserve_detail")
			{
				//just some wild suggestion for parameter settings
				params.noiselevel.continuous_estimate = true;
				params.alpha = 0.0f;
				params.noiselevel.stds_from_mean = 4.f;
	...
	//configure GPU
	/////////////////////////////////////////////////////////////////////
	denoise::IterativeNLM_GPU iternlm_gpu;

	if (params.gpu.n_gpus > 0)
	{
		int max_parallelslices_gpu = iternlm_gpu.configure_device(shape, &params);
		if (max_parallelslices_gpu == -2) return -2; //error
		else if (max_parallelslices_gpu == -1) cout << "\033[1;31mWarning! No Nvidia-GPU found. Switching to CPU-mode.\033[0m" << endl;
		else if (max_parallelslices_gpu < shape[2] && params.gpu.n_gpus == 1) cout << "\033[1;33mData exceed GPU capacity. Processing blocks of " << max_parallelslices_gpu << " slices\033[0m" << endl;
		else if (max_parallelslices_gpu < shape[2]) cout << "Processing blocks of " << max_parallelslices_gpu << " slices" << endl;

		if (params.gpu.n_gpus > 0 && params.gpu.blockwise_host == false) params.cpu.blockwise = false;
	}
	/////////////////////////////////////////////////////////////////////
	...
	if ("get initial noise level"){
		...
		if (params.io.resume)
			estimate_available = hdcom.ReadNoisefile(sigma0, shape, outpath + "/noise_vs_slice"+aux::zfill_int2string(0,2)+".csv");
		...
		if (!estimate_available)
		{
			...
			if(!params.io.cleanup || verbose)
				hdcom.SaveNoisefile(sigma0, shape[2], outpath, "noise_vs_slice"+aux::zfill_int2string(0,2));
		}
		else cout << "Read in initial noise level from file!" << endl;

		if (params.gpu.n_gpus > 0) iternlm_gpu.set_sigma(sigma0, shape);
	...
	if (params.gpu.n_gpus > 0)
	{
		if (!params.gpu.blockwise_host && iternlm_gpu.n_parallelslices >= shape[2])
		{
			//assuming full stack is read in on host
			output = (float*) malloc(nstack*sizeof(*output));
		...
		else if (!params.gpu.blockwise_host)
		{
			//assuming full stack is read in on host
			output = (float*) malloc(nstack*sizeof(*output));
		...
		{
			iternlm_gpu.Run_GaussianNoise_GPUBlocks(1, filelist, filelist, shape, &params);
	...
	if ("get texture level" && params.maxiterations > 1){
		...
		bool estimate_available = false;
		if (params.io.resume)
			estimate_available = hdcom.ReadNoisefile(sigma1, shape, outpath + "/noise_vs_slice"+aux::zfill_int2string(1,2)+".csv");
		...
		if (!estimate_available)
		{
			...
			if(!params.io.cleanup || verbose)
				hdcom.SaveNoisefile(sigma1, shape[2], outpath, "noise_vs_slice"+aux::zfill_int2string(1,2));
		}
		else cout << "Read in texture level from file!" << endl;
		...
		//set on gpu
		if (params.gpu.n_gpus > 0) iternlm_gpu.set_sigma(sigma1, shape);
	}
	/////////////////////////////////////////////////////////////////////
	...
	for (int iter = 2; iter <= params.maxiterations; iter++)
	{
		...
		if (params.gpu.n_gpus > 0)
		{
			if (!params.gpu.blockwise_host && iternlm_gpu.n_parallelslices >= shape[2])
			{
				iternlm_gpu.Run_GaussianNoise(iter, instack, shape, &params);

				if((!params.io.cleanup || params.maxiterations == iter) && !iternlm_gpu.resumed){
					iternlm_gpu.get_result(output, shape);
					if (params.io.save_type == "16bit") hdcom.SaveTifSequence_as16bit(params.io.firstslice, output, shape, params.io.active_outpath, "denoised", false);
					else hdcom.SaveTifSequence_32bit(params.io.firstslice, output, shape, params.io.active_outpath, "denoised", false);
				}
			...
			{
				iternlm_gpu.Run_GaussianNoise_GPUBlocks(iter, instack, output, shape, &params);
			...
			{
				iternlm_gpu.Run_GaussianNoise_GPUBlocks(iter, filelist, filelist_prev, shape, &params);
				filelist_prev = hdcom.GetFilelist(params.io.active_outpath, shape);
			}
		}
		else
		...
		if (params.noiselevel.continuous_estimate && params.maxiterations > iter+1){

			bool estimate_available = false;
			if (params.io.resume)
				estimate_available = hdcom.ReadNoisefile(sigma1, shape, outpath + "/noise_vs_slice"+aux::zfill_int2string(iter,2)+".csv");
			...
			if (!estimate_available)
			{
				...
				if(!params.io.cleanup || verbose)
					hdcom.SaveNoisefile(sigma1, shape[2], outpath, "noise_vs_slice"+aux::zfill_int2string(iter,2));
			}
			else cout << "Read in texture level from file!" << endl;

			cout << "noise level " << iter << " (slice " << shape[2]/2 << "): " << sigma1[shape[2]/2] << "          " << endl;
